...
public class QueryGenerator {
	...
	public static void PopulateSelect(){
		Iterator<String> itr = DivideList.projection.iterator();
		while(itr.hasNext()){
			...
			if(temp.length > 1){
				...
				if(tempHold.size() > 0){
					...
					System.out.println(operator+"i got nothing");
	...
	/**
	 * This function explicitly uses the selection list produced by the
	...
	public static void PopulateWhere(){
		...
		/**
		 * Separate the selection list in right and left side.
		 * */
		Iterator<String> itr = DivideList.selection.iterator();
		while(itr.hasNext()){
	...
	public static ArrayList<String> ProcessRight(ArrayList<String> list){
		...
		if(list.size() == 1){
			...
			for(int i = 0; i < list.size(); i++){
				if(DivideList.projection.contains(list.get(i).trim())){
				}
	...
	public static ArrayList<String> ProcessLeft(ArrayList<String> list){
		...
		if(list.size() < 1){
			...
			for(int j = 0; j < list.size(); j++){
				if(DivideList.projection.contains(list.get(j).trim())){
				}
				else if(QueryGenerator.chkProjection(list.get(j).trim())){
				}
				else{
					temp.add(list.get(j));
				}
	...
	public static boolean chkProjection(String str){
		boolean status = false;
		for(int j = 0; j < DivideList.projection.size(); j++){
			if(DivideList.projection.get(j).endsWith(str)){
				status = true;
				break;
			}
		}
	...
	public static String GenerateSelect(){
		...
		if(select.size() < 1){
			try{
				...
				JOptionPane.showMessageDialog(null, noise, "Error", JOptionPane.ERROR_MESSAGE, new ImageIcon("icons\\error.png"));
			}
		}
