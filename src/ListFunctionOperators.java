...
public class ListFunctionOperators {
	public static JTable listSyms;
	public static String[] holdStr = {"S.n","Symonym","Function or Operator","Clause"};
	public static String[] flds = {"fsynonym","operator","type"};
	public static TableClass tblcls = new TableClass(holdStr,flds,"select fsynonym,operator,type from funcop");
	
	public JPanel AllGui(){
		JPanel temp = new JPanel();
		temp.setOpaque(false);
		temp.setLayout(new BorderLayout());
		JLabel icons = new JLabel(new ImageIcon("icons\\list-func-panel.png")); 
	...
	public JPanel comBined(){
		JPanel temp = new JPanel();
		temp.setLayout(new BorderLayout());
		temp.add(this.AllGui(), BorderLayout.NORTH);
	...
	public JPanel ListTable (){
		...
		listSyms = new JTable(tblcls){
			private static final long serialVersionUID = 907292630688259601L;
	...
	public class hdlevent extends MouseAdapter{
		...
		public void mouseClicked(MouseEvent event) {
			...
			if(listSyms.isRowSelected(row)){
				if(test.equals("edit")){
					UpdateFunctionOperators.txtSym.setText((String)listSyms.getValueAt(row, 1));
					UpdateFunctionOperators.txtOP.setText((String)listSyms.getValueAt(row, 2));
					UpdateFunctionOperators.box.setSelectedIndex((((String)listSyms.getValueAt(row, 3) == "select") ? 1 : 2));
					UpdateFunctionOperators.curRow = row;
					UpdateFunctionOperators.preValue = (String)listSyms.getValueAt(row, 1);
					AllGui.layout.show(AllGui.allPanels, "UpdateFun");
				}
					...
					if(i == JOptionPane.OK_OPTION){
						if(SqlLibrary.DeleteRow("funcop", "fsynonym like '"+listSyms.getValueAt(row, 1)+"%'")){
							tblcls.deleteRow(row);
							JOptionPane.showMessageDialog(null, "The Function Operator is deleted successfully.", "Success", JOptionPane.ERROR_MESSAGE, new ImageIcon("icons\\correct.png"));
						}
						}
				}
			}
