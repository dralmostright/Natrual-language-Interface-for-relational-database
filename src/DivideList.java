 ...
 * i.e First list consists of projection which are probably 
 * used for selection in select statement
 * And Second consists of the selection which are probably 
 * user for the where clause
 */

public class DivideList {
	/**
	 * The selection list is used for building the where clause
	 * The projection list is used for building the select clause
	 * */
	public static ArrayList<String> selection; // W
	public static ArrayList<String> projection; //S 
		...
		while(processedList.size() > 0){
			...
			while(itr.hasNext()){
				...
				//Rule 2
					...
					//sub rule i
					if(HasElement(selection, temp[0].trim())){
						projection.add(temp[1].trim());
						//System.out.println("1 added: nsubj: projection:"+temp[1]);
						itr.remove();
					}
					
					//sub rule ii 
					//-- The rule is modified by not checking the hasRoot flag is either set or not set.
					else if((!HasElement(selection, temp[0].trim())) && hasRoot == false){
						selection.add(temp[0].trim());
						projection.add(temp[1].trim());
						//System.out.println("2 added: nsubj: projection:"+temp[1]+" selection:"+temp[0]);
						itr.remove();
					}
				...
				//rule 3
					...
					if(SqlLibrary.ChkOperatorStatus(temp[0])){
						...
						if(test.equals("sel")){
							projection.add(temp[0].trim()+"-"+temp[1].trim());
							itr.remove();
						}
						else if(test.equals("whe")){
							selection.add(temp[0].trim()+"-"+temp[1].trim());
							itr.remove();
						}
					}
					 ...
					 * */
					else if(HasElement(projection,temp[0].trim())){
						if(!SqlLibrary.ChkStatus(temp[0].trim(), temp[1].trim())){
							selection.add(temp[1].trim());
							//System.out.println("1.1 added: prep or obj: selection:"+temp[1]);
						}
						else{
							//System.out.println("1.2 added: prep or obj: selection:"+temp[1]+"Projection:"+temp[1]);
							selection.add(temp[1].trim());
							projection.add(temp[1].trim());
						}
...
//							selection.add(temp[1]);
//							projection.add(temp[0]);
//							itr.remove();
//							System.out.println("2.1 added: prep or obj: selection:"+temp[1]+"Projection: "+temp[0]);
//						}
//						else if(!SqlLibrary.ChkStatus(temp[0], temp[1])){
//							selection.add(temp[1]);
//							projection.add(temp[0]);
//							System.out.println("2 added: prep or obj: selection:"+temp[1]+"Projection: "+temp[0]);
//							itr.remove();
//						}
						//else{
							selection.add(temp[1].trim());
							projection.add(temp[0].trim());
							itr.remove();
						//}
				 ...
				 * */
					...
					//sub rule ii
						...
						if(test.equals("sel")){
							projection.add(temp[1].trim()+"-"+temp[0].trim());
							itr.remove();
						}
						else if(test.equals("whe")){
							selection.add(temp[1].trim()+"-"+temp[0].trim());
							itr.remove();
						}
				...
				//rule 7
					...
					String[] temp = DivideList.GetGovDep(split[1]);
					if(projection.contains(temp[1].trim())){
						itr.remove();
					}
